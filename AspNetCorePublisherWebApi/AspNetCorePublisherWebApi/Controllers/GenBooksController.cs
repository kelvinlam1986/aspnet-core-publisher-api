...
namespace AspNetCorePublisherWebApi.Controllers
{
    ...
    public class GenBooksController : Controller
    {
        ...
        {
            var items = _rep.Get<Book>().Where(x => x.PublisherId.Equals(publisherId));
        ...
        public IActionResult Post(int publisherId, [FromBody] BookDTO DTO)
        {
            ...
            return CreatedAtRoute("GetGenericBook", new { id = createdDTO.Id }, createdDTO);
        }
        ...
        {
            if (!_rep.Exist<Book>(id)) return NotFound();
